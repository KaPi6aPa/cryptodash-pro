...
import { Search, AlertCircle, Loader2, ChevronUp, ChevronDown } from 'lucide-react';

type SortKey = 'name' | 'current_price' | 'price_change_percentage_24h' | 'total_volume' | 'market_cap';
...
export const MarketPage = () => {
  ...
  const SortIcon = ({ column }: { column: SortKey }) => {
    ...
    return sortConfig.direction === 'asc'
      ? <ChevronUp className="inline h-4 w-4 text-emerald-400" />
      : <ChevronDown className="inline h-4 w-4 text-emerald-400" />;
  };
            ...
            <table className="w-full min-w-full text-left border-collapse">
              <thead className="bg-slate-900/50 sticky top-0">
                <tr>
                  ...
                  <th
                    className="p-4 text-slate-400 font-medium text-sm cursor-pointer select-none hover:text-white transition-colors"
                    onClick={() => handleSort('name')}
                  >
                    Asset <SortIcon column="name" />
                  </th>
                  <th
                    className="p-4 text-slate-400 font-medium text-sm text-right cursor-pointer select-none hover:text-white transition-colors"
